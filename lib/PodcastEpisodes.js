...
export default class PodcastEpisodes extends events.EventEmitter {
  constructor(feed) {
    super();
    this.feed = feed;
    this.podcasts = [];
  ...
  _FromRSS(podcast) {
    return parser
      ...
      .then((feed) => {
        ...
        this.podcasts = p.slice(0, podcast.recent || 3);
  ...
  Start() {
    const self = this;
    return this._FromRSS(this.feed)
      .then((d) => {
        if (Array.isArray(d)) {
